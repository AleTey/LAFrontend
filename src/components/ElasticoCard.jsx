...
export const ElasticoCard = ({ elastico }) => {
  ...
  const [editElasticoIsOpen, setEditElasticoIsOpen] = useState(false);

  const { deleteElastico } = useElastico();

  const { onAddOrderList } = useOrders();


  const onDeleteElastico = (id) => {
  ...
  const onEditElastico = () => {
    toggle();
    modalSelectionHandler("editElastico")
    setEditElasticoIsOpen(true);
  }
  ...
  return (
    <>
      ...
      {
        // editElasticoIsOpen && inputModalIsOpen && selectedModal === "editElastico" &&
        editElasticoIsOpen && selectedModal === "editElastico" &&
        <NewInputModal
          inputToEdit={elastico}
          formIsOpen={setEditElasticoIsOpen}
        />
      }
      ...
      <div className="card d-flex mx-2 my-2" style={{ width: " 22rem" }} >
        <div className="card-body">
          ...
          <ul className="list-group list-group-flush">
            ...
            <li className="list-group-item"><b>Codigo:</b> {elastico.codigo}</li>
        ...
        <div className="card-body grid">
          ...
          <abbr title="Agregar a pedidos" className="initialism">
            <button className="btn btn-success mx-1" onClick={() => onAddOrderList(elastico)}>
          ...
          <abbr title="Editar" className="initialism">
            <button className="btn btn-secondary mx-1" onClick={onEditElastico} >
          ...
          <abbr title="Eliminar insumo" className="initialism">
            <button className="btn btn-danger mx-1" onClick={() => onDeleteElastico(elastico.id)}>
